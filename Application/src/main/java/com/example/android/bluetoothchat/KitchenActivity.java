...
public class KitchenActivity extends AppCompatActivity {
    ...
    public void onoff(ImageButton bt, int check){
        if(check == 0) {//CHECK_NUM 이0 일경우 setSelected를 true로 줘서 초록스위치가 나오게 한다
            bt.setSelected(true);
            check = 1; // 다음에 누르면 색이 변하도록 값을 변경.
        }
        else //CHECK_NUM 0이 아니면 setSelected를 false로 줘서 빨간 스위치가 나오게 한다.
        {
            bt.setSelected(false);
            check = 0; //다음에 누르면 색이 변하도록 값을 변경
        }
